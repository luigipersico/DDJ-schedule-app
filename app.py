...
# --- FEATURE FLAG ---
# If missing from secrets (like on the live cloud), it defaults to False (hidden).
ENABLE_TOURS = st.secrets.get("ENABLE_TOURS", False)
...
st.title("SPC DDJ Portal 3.0" if ENABLE_TOURS else "SPC DDJ Portal")
...
if ENABLE_TOURS:
    ...
    main_tab1, main_tab2 = st.tabs(["📝 My Availability", "📅 DDJ History"])
...
with main_tab1:
    ...
    if existing_away_dates:
        ...
        for i, away_period in enumerate(existing_away_dates):
            is_past = False
            try:
                if datetime.date.fromisoformat(away_period['end']) < today_date: is_past = True
            except: pass 
...
if ENABLE_TOURS:
    with main_tab3:
        ...
        guide_counts = {name: data.get("historical_tours", 0) for name, data in db.items() if name not in ["__CONFIG__", "__TOURS__", "__GUEST_GUIDES__"] and data.get("historical_tours", 0) > 0}
                    ...
                    if st.button("Generate Email Draft"):
                        ...
                        for t in tours_db:
                            try:
                                ...
                                if datetime.date.today() <= t_date <= next_week and not t.get("counted", False):
                        ...
                        if not empty_tours: 
                            st.success("All tours for the next 7 days are fully booked!")
                        else:
                            subject = urllib.parse.quote("Urgent: Tour Guides Needed for Upcoming Visits")
                            body = "Dear All,\n\nThis is a friendly reminder to fill out the Doodle poll if you have not yet had the chance to do so.\nTo sign up, please use this link: [ here ]\nList of upcoming visits: [ here ]\n\nWe are currently in urgent need of additional guides for the groups listed below. If you are available to support any of these visits, please let me know.\n\n"
                            
                            lang_map = {"English": "EN", "French": "FR", "German": "DE", "Italian": "IT", "Spanish": "ES", "Chinese": "ZH"}
                            ...
                            for t in empty_tours:
                                try:
                                    ...
                                    time_obj = datetime.datetime.strptime(t["time"], "%H:%M:%S")
                                    dur_parts = t.get("duration", "00:45").split(":")
                                    end_time = time_obj + datetime.timedelta(hours=int(dur_parts[0]), minutes=int(dur_parts[1]))
                                    time_str = f"{time_obj.strftime('%H:%M')} – {end_time.strftime('%H:%M')}"
                                    
                                    missing = t.get("guides_needed", 1) - (len(t.get("assigned_guides", [])) + len(t.get("approved_guides", [])) + len(t.get("guest_guides", [])))
                                    guide_word = "guide" if missing == 1 else "guides"
                                    lang_code = lang_map.get(t.get("language", "English"), "EN")
                                    
                                    body += f"{date_str}, {time_str}\n{t['group']}\n{missing} {guide_word} {lang_code}\n\n"
                                except Exception:
                                    pass
                            
                            body += "Thank you very much for your time, support, and continued dedication.\n\nKind regards,\n\nDace"
                            
                            mailto_link = f"mailto:spc-guides@groupes.epfl.ch?subject={subject}&body={urllib.parse.quote(body)}"
        ...
        with tour_tab2:
            ...
            if not years_available:
                ...
                guide_counts_year = {}
                for t in year_tours:
                    for g in t.get("approved_guides", []): guide_counts_year[g] = guide_counts_year.get(g, 0) + 1
                    for g in t.get("guest_guides", []): guide_counts_year[g] = guide_counts_year.get(g, 0) + 1
                    
                if guide_counts_year:
                    current_year_str = str(datetime.date.today().year)
                    months_passed = datetime.date.today().month if selected_year == current_year_str else 12
                    
                    table_data = []
                    for guide, count in sorted(guide_counts_year.items(), key=lambda x: x[1], reverse=True):
